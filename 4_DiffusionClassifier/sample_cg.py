...
classifier.eval()
classifier.load_state_dict(torch.load('4_DiffusionClassifier/weights/classifier/model.pth', map_location=device))

unet = UNet(device=device)
unet.eval()
unet.to(device)
unet.load_state_dict(torch.load('4_DiffusionClassifier/weights/DDPM/model.pth', map_location=device))
...
show(imgs, fig_titles=CLASS_LABELS, title=None, save_path='assets/cg_samples.pdf')
