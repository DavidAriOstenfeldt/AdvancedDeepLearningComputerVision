...
if __name__ == '__main__':
    ...
    ########################################### classifier guidance ##########################################
    ddpm_cg = Diffusion(img_size=16, T=500, beta_start=1e-4, beta_end=0.02, diff_type='DDPM', device=device)
    ...
    unet_ddpm.load_state_dict(torch.load('models_ex3/ddpm/weights-30.pt', map_location=device))
    ddpm_cg.classifier = classifier

    ######################################### classifier-free guidance #########################################
    ddpm_cFg = Diffusion(img_size=16, T=500, beta_start=1e-4, beta_end=0.02, diff_type='DDPM', device=device)
    unet_ddpm_cFg = UNet(device=device)
    unet_ddpm_cFg.eval()
    unet_ddpm_cFg.to(device)
    # unet_ddpm_cFg.load_state_dict(torch.load('weights/DDPM-cfg/model.pth', map_location=device))
    unet_ddpm_cFg.load_state_dict(torch.load('models_ex3/ddpm-alldata/weights-30.pt', map_location=device))
