...
if __name__ == '__main__':
    ...
    model.to(device)
    model.eval()
    model.load_state_dict(torch.load('4_DiffusionClassifier/weights/classifier/model.pth', map_location=device))
    ...
    plt.figure()
    plt.title(f'Per Timestep Accuracy, max: {np.round(per_t_acc.cpu().numpy()[0]*100, 2)}%', fontweight='bold')
    plt.plot(t.cpu().numpy(), per_t_acc.cpu().numpy(), linewidth=3)
    plt.xlabel('Timestep', fontweight='bold')
    plt.ylabel('Accuracy', fontweight='bold')
    plt.savefig('./assets/per_t_accuracy.pdf', bbox_inches='tight')
    ...
    show_n_forward(vis_images, fig_titles=vis_labels, save_path='assets/clf_forward.pdf')
