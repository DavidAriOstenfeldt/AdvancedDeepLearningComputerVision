...
def train(device='cpu', T=500, img_size=16, input_channels=3, channels=32, time_dim=256):
    ...
    for epoch in pbar:
        ...
        for images, labels in train_loader:
            ...
            accuracy = (predicted_class.argmax(dim=-1) == labels).float().mean()
            pbar.set_postfix(CE=loss.item(), accuracy=accuracy.item())
        ...
        for images, labels in val_loader:
            ...
            x_t, noise = diffusion.q_sample(images, t)
            predicted_class = model(x_t, t)
            val_loss = loss_fn(predicted_class, labels)
            val_accuracy = (predicted_class.argmax(dim=-1) == labels).float().mean()
            pbar.set_postfix(CE=loss.item(), accuracy=accuracy.item(), val_CE=val_loss.item(), val_accuracy=val_accuracy.item())
        
    # save your checkpoint in weights/classifier/model.pth
    torch.save(model.state_dict(), os.path.join("weights", exp_name, 'model_yee.pth'))
