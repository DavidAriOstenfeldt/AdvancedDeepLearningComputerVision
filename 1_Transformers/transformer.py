...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        keys = rearrange(keys, 'b s (nh hd) -> b s nh hd ', nh=self.num_heads, hd=self.head_dim)
        ...
        queries = rearrange(queries, 'b s (nh hd) -> b s nh hd ', nh=self.num_heads, hd=self.head_dim)
        queries = rearrange(queries, 'b s nh hd -> (b nh) s hd')

        values = rearrange(values, 'b s (nh hd) -> b s nh hd ', nh=self.num_heads, hd=self.head_dim)
        values = rearrange(values, 'b s nh hd -> (b nh) s hd')

        # Compute attetion logits
        attention_logits = queries * keys # multiply queries and keys
        ...
        assert attention.size() == (batch_size*self.num_heads, seq_length, self.head_dim)
